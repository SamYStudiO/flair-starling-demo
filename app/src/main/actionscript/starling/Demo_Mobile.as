package starling
{
	...
	public class Demo_Mobile extends Sprite
	{
		...
		private function loadAssets( scaleFactor : int , onComplete : Function ) : void
		{
			...
			assets.enqueue( appDir.resolvePath( "audio" ) , appDir.resolvePath( StringUtil.format( "fonts/{0}x" , scaleFactor ) ) , appDir.resolvePath( StringUtil.format( "textures/{0}x" , scaleFactor ) ) );
		...
		private function initElements( scaleFactor : int ) : void
		{
			...
			_background.contentLoaderInfo.addEventListener( flash.events.Event.COMPLETE , function ( e : Object ) : void
			{
